...
- AI Configuration: Adjustable response style, length, and tone; knowledge base customization`;
  ...
  app.get("/api/platforms/:id/analytics", authMiddleware, async (req, res) => {
    try {
      const platformId = parseInt(req.params.id);
      console.log(`Getting analytics for platform ID: ${platformId}`);
      ...
      // Check authorization (same as other platform routes)
      if (platform.userId !== req.user.id && !(platformId === 3 && platform.type === "discord")) {
        return res.status(403).json({ message: "Unauthorized" });
      }
      
      // Generate 14 days of historical data for demo purposes
      ...
      for (let i = 13; i >= 0; i--) {
        ...
        // For the Discord platform, generate more interesting data
        if (platform.type === "discord") {
          // Base values
          let messageCount = Math.floor(Math.random() * 30) + 50; // 50-80 messages
          let moderationCount = Math.floor(Math.random() * 5) + 1; // 1-6 moderation actions
          let userCount = Math.floor(Math.random() * 10) + 110; // 110-120 users
          
          // Add trend - messages increasing over time
          messageCount = Math.floor(messageCount * (1 + (i / 30)));
          
          // Weekend dip - messages decrease on weekends
          const day = date.getDay();
          if (day === 0 || day === 6) { // Saturday or Sunday
            messageCount = Math.floor(messageCount * 0.7);
          }
          
          // Moderation increases with messages 
          moderationCount = Math.floor(messageCount / 20);
          ...
          historicalData.push({
            ...
            moderationCount,
            userCount,
            activeChannels: platform.config?.channels?.length || 5
          });
        } else {
          // For other platforms, use simpler data
          historicalData.push({
            date: date.toISOString().split('T')[0],
            messageCount: Math.floor(Math.random() * 50) + 10,
            moderationCount: Math.floor(Math.random() * 3) + 1,
            userCount: Math.floor(Math.random() * 20) + 50
          });
        }
      }
      
      // Add current metrics from platform config
      const currentMetrics = {
        date: now.toISOString().split('T')[0],
        messageCount: platform.config?.dailyMessages || 0,
        moderationCount: platform.config?.moderationCount || 0,
        userCount: platform.config?.memberCount || 0,
        channelCount: platform.config?.channels?.length || 0
      };
      
      // Calculate totals for the summary metrics
      const totalMessages = historicalData.reduce((sum, day) => sum + day.messageCount, 0) + currentMetrics.messageCount;
      const totalModerationActions = historicalData.reduce((sum, day) => sum + day.moderationCount, 0) + currentMetrics.moderationCount;
      ...
      return res.status(200).json({
        ...
        summary: {
          ...
          activeUsers: currentMetrics.userCount,
          activeChannels: currentMetrics.channelCount,
          period: "Last 14 days"
        }
      });
    } catch (error) {
      console.error(`Error fetching analytics for platform ${req.params.id}:`, error);
